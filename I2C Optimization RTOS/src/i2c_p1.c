...
// ISR
int volatile isr_state = 0;
int volatile isr_finished = 0;
int volatile txrx;
uint8_t volatile dummy_isr, num_bytes_read_isr=0, is_last_read_isr=0;
...
void I2C0_IRQHandler(void) {
	...
	I2C0->S |= I2C_S_IICIF_MASK;
	if (i2c_mode == 2) {
		osSignalSet(t_I2C, I2C_SIGNAL);
	}
	else if (i2c_mode == 3) {
		switch (isr_state) {
			case 0:
			I2C0->D = reg_adx_isr;												//	send register address	
			isr_state = 1;
			break;
			
			case 1:
			I2C_M_RSTART;															//	repeated start									
			I2C0->D = dev_adx_isr | 0x01 ;								//	send dev address (read)		
			isr_state = 2;
			break;
...
int i2c_read_bytes_isr_bytes(uint8_t dev_adx, uint8_t reg_adx, uint8_t * data, uint8_t data_count) {
	...
	I2C0->D = dev_adx;												//	send dev address (write)	
	osSignalWait(I2C_SIGNAL, osWaitForever);	//	wait for completion	
